...
redcapExport <- function(APIKEY, URI='https://redcap.vanderbilt.edu/api/', labels=TRUE, checkboxLabels=FALSE, forms=NULL, fields=NULL, events=NULL) {
  ...
  # if sjlabelled is available, apply labels
  sjlab <- require('sjlabelled')
  ...
  for (i in seq_len(nrow(meta_data))) {
    ...
    if (fld$field_type == 'checkbox') {
      ...
      for (j in seq(length(nums))) {
        ...
        if (sjlab) {
          set_label(data[[checkbox_name]]) <- sprintf('%s (choice=%s)', clean(fld$field_label), rmq(choices[j]))
        }
      }
      ...
      if (sjlab) {
        set_label(data[[fld$field_name]]) <- fld$field_label
      }
    }
  }
  ...
  for (form_field_name in form_field_names) {
    ...
    if (sjlab) {
      set_label(data[[form_field_name]]) <- 'Complete?'
    }
  }
